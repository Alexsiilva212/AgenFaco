...
// Persistência
// -----------------------------------------------------------------------------
// O app conversa com o backend por estas quatro funções, chamando os
// endpoints /api/base e /api/senhas (arquivos em /api). Hoje esses
// endpoints são só um contrato (placeholder) — veja api/base.js e
// api/senhas.js para ligar o banco de dados de verdade.
// ---------------------------------------------------------------------------
...
async function loadBase() {
  try {
    ...
    return data && data.pacientes ? data : null;
  } catch (e) {
    return null;
  }
}

async function saveBase(payload) {
  try {
    const res = await fetch("/api/base", {
      method: "POST",
...
async function loadSenhas() {
  try {
    ...
    if (!res.ok) return null;
    const data = await res.json();
    return data && data.admin ? data : null;
  } catch (e) {
    return null;
  }
}

async function saveSenhas(payload) {
  try {
    const res = await fetch("/api/senhas", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
function MenuScreen({ pacientes, onNavigate }) {
  ...
  return (
    <div style={{ minHeight: "100%", background: "#F3F7F6" }}>
      ...
      <div className="no-print menu-cards" style={{ padding: "20px 16px 32px" }}>
        {cards.map((c) => (
          <button
            key={c.key}
...
function AdminPanel({ pacientes, onCarregarBase, onBack, ultimaCarga, senhas, onSalvarSenhas }) {
  ...
  return (
    <div style={{ minHeight: "100%", background: "#F3F7F6", paddingBottom: 40 }}>
      ...
      <div className="no-print admin-layout" style={{ padding: 16 }}>
       ...
       <div className="admin-right">
        ...
        <div style={{ background: "#FFFFFF", border: "1px solid #E3EBE9", borderRadius: 16, padding: 16, marginTop: 14 }}>
          ...
          <div className="card-grid" style={{ marginTop: 6, maxHeight: 480, overflowY: "auto" }}>
...
export default function AgendaFacoApp() {
  ...
  useEffect(() => {
    (async () => {
      const [dados, senhasSalvas] = await Promise.all([loadBase(), loadSenhas()]);
      if (dados) {
  ...
  const persistirBase = useCallback(
    async (novaLista, novaCarga) => {
      ...
      const ok = await saveBase(payload);
  ...
  return (
    <div className="app-shell" style={{ minHeight: "100vh", background: "#F3F7F6", fontFamily: "'IBM Plex Sans', sans-serif", boxShadow: "0 0 40px rgba(14,67,64,0.06)" }}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        /* Largura do app: cresce em telas maiores em vez de ficar preso a 480px */
        .app-shell { max-width: 480px; margin: 0 auto; width: 100%; }
        @media (min-width: 700px) { .app-shell { max-width: 720px; } }
        @media (min-width: 1040px) { .app-shell { max-width: 1160px; } }

        /* Cartões do menu: 1 coluna no celular, lado a lado em telas largas */
        .menu-cards { display: grid; grid-template-columns: 1fr; gap: 12px; }
        @media (min-width: 700px) { .menu-cards { grid-template-columns: repeat(3, 1fr); } }

        /* Listas de pacientes: 1 coluna no celular, várias colunas conforme o espaço */
        .card-grid { display: grid; grid-template-columns: 1fr; gap: 10px; }
        @media (min-width: 640px) { .card-grid { grid-template-columns: repeat(2, 1fr); } }
        @media (min-width: 1040px) { .card-grid { grid-template-columns: repeat(3, 1fr); } }

        /* Administração: empilhado no celular, colunas lado a lado em telas largas */
        ...
        @media (min-width: 1040px) {
          ...
          .admin-layout .admin-left { display: flex; flex-direction: column; }
        }
      `}</style>
